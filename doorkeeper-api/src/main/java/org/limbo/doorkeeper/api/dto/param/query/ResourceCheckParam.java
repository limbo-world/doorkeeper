...
public class ResourceCheckParam {
    ...
    @NotNull(message = "请选择命名空间")
    @Parameter(description = "进行权限校验时，资源所属命名空间", required = true)
    ...
    public void andTag(String key, String value) {
        if (andTags == null) {
            andTags = new ArrayList<>();
        }
        andTags.add(key + ApiConstants.KV_DELIMITER + value);
    }

    public void orTag(String key, String value) {
        if (orTags == null) {
            orTags = new ArrayList<>();
        }
        orTags.add(key + ApiConstants.KV_DELIMITER + value);
    }
