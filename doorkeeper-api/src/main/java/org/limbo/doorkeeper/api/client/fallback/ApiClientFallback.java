...
/**
 * @author Devil
 * @date 2020/11/23 4:48 PM
 */
@Slf4j
@Component
public class ApiClientFallback extends Fallback implements FallbackFactory<ApiClient> {
    @Override
    public ApiClient create(Throwable throwable) {
        log.error("服务调用失败", throwable);
        return new ApiClient() {
            @Override
            public Response<ApiVO> add(ApiAddParam param) {
                return serviceUnavailable();
            }

            @Override
            public Response<Integer> update(Long apiId, ApiUpdateParam param) {
                return serviceUnavailable();
            }
