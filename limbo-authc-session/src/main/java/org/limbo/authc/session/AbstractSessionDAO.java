...
import org.limbo.authc.session.utils.UUIDUtils;
...
public abstract class AbstractSessionDAO<T extends AbstractSession> {
    ...
    public T createSession(SessionAccount sessionAccount) {
        try {
            ...
            securityDigest.setPublicKey(publicKey);
            securityDigest.setPrivateKey(privateKey);

            // 会话用户
            T session = create(UUIDUtils.get(), securityDigest, sessionAccount);
     ...
     */
    protected abstract T create(String sessionId, SecurityDigest securityDigest, SessionAccount sessionAccount);
