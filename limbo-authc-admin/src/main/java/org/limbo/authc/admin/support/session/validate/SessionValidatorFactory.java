...
public class SessionValidatorFactory {
    ...
    public static SessionValidator create(AuthcProperties authcProperties, HttpServletRequest request) {
        // 目前只用header校验
        String headerVName = HeaderSessionValidator.class.getName();
        validators.computeIfAbsent(headerVName, k -> new HeaderSessionValidator(authcProperties));
